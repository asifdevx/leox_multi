...
import { findByRole,findUser } from "../controllers/userInfo.controlers";

const router = express.Router();

router.post("/findByAddress",async(req:Request,res:Response)=>{
  try {
...
router.post("/findByRole",async(req:Request,res:Response)=>{
  try {
    const {role} = req.body;
